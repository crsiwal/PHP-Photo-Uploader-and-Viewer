...
const App = () => {
  ...
  const fetchImages = async () => {
    ...
    setLoading(true);
    try {
      // const url = "http://web6.adgyde.in"; // Development
      // const url = "https://gaurav.gchat.in/services"; // Live
      const url = "https://picsum.photos/v2";

      const response = await fetch(`${url}/list?page=${page}&limit=${imageLimit}`); // Development
      // const response = await fetch(`${url}/api.php?page=${page}&limit=${imageLimit}`); // Live
      ...
      if (data) {
        ...
        setHasMore(data.length === imageLimit);
      }
    } catch (error) {
  ...
  return (
    <div className="container mt-4">
      <div className="row gy-4">
        {images?.map(image => (
          <div key={`photo_${image.id}`} className="col-12">
            <div className="card">
              <LazyLoadImage
